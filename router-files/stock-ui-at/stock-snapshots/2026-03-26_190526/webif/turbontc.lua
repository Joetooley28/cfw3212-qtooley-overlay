...
local authGroupConf = require("userGroupAuth")
local AUTH_TABLE = authGroupConf.authTable
local SessionRequestHandler = require("session_handler")
local language_selection = require("language_selection_handler")
local top_menu_entries = require("top_menu_entries")
...
formUserPass = "password"

local defaultPageAfterLogin = config.DEFAULT_PAGE

local loginAccounts
local loginAccountsTbl = {}

function isUsernameValid(username)
    return loginAccountsTbl[username]
end
...
local function getStoredPassword(username)
    ...
    turbo.log.error("Failed to get stored password")
    return nil
end
...
local function verifyPassword(password, encryptedPassword)
    ...
    local cmd = string.format("echo -n '%s' | openssl enc -a -A -d | openssl passwd -stdin -%s -salt '%s'",
                              b64pw, alg, salt)
...
function validateUser(userInfo)
    ...
    turbo.log.debug("Failed to validate user")
    return false
end

function assertValidUrl(url)
    if not AUTH_TABLE[url] then
        turbo.log.error("URL "..url.." is not valid")
        error(turbo.web.HTTPError(404))
    end
end

function needsAuthentication(url)
    if AUTH_TABLE[url].skipAuth then
        turbo.log.debug("URL "..url.." allows pre-login access")
        return false
    end
    return true
end

function assertValidLogin(appHandler)
    local session = appHandler.session
    if not csrf_session.isSessionValid(session) then
        turbo.log.error("Got invalid session")
        error(turbo.web.HTTPError(401))
    end
    if not session.user then
        turbo.log.error("User is not logged in")
        error(turbo.web.HTTPError(401))
    end
end

function assertValidGroup(appHandler, url, method, check)
    if not AUTH_TABLE[url] then
        error(turbo.web.HTTPError(404))
    end
    local userGroups = getUserGroups(appHandler)
    local allowedGroups = method == "get" and AUTH_TABLE[url].readGroups or AUTH_TABLE[url].writeGroups
    if not allowedGroups then
        turbo.log.error("URL "..url.." allowed groups not found")
    ...
    turbo.log.error("User group authorisation failed")
    if check then
        return false
    else
        error(turbo.web.HTTPError(403))
...
function assertValidCsrfToken(appHandler, csrfToken)
    if not csrf_session.verifyCsrfToken(appHandler.session.csrf, csrfToken) then
        turbo.log.error("Invalid csrf token")
        error(turbo.web.HTTPError(403))
    end
end

-- validate Host and Referer in request headers to prevent dns binding attacks
local function validateHost(appHandler)
    -- get Host and Referer in request headers
    local Host = appHandler.request.headers:get("Host")
    local Referer = appHandler.request.headers:get("Referer")
    -- parse ip
    local removePort = string.find(Host,":")
    if removePort then
        Host = string.sub(Host, 1, removePort - 1)
    end
    -- verify if Host is an ip
    if(isValid.IpAddress(Host) == false) then
        return false
    end
    -- verify Referer
    if(Referer and string.find(Referer, Host) == nil) then
        -- If the lan ip is changed or factory reset, the host verification is passed and
        -- the page should be redirected to the login page.
        local lan_ip = luardb.get(getDefaultRouteLanRdbs().rdbAddrPrefix.."address") or ""
        if (Host == lan_ip) then
            return true
        end
        return false
...
function LoginHandler:post(url)
    ...
    if not csrf_session.verifyCsrfToken(loginMsg["csrfToken"], csrfToken) then
        ...
        self:write(response)
        csrf_session.updateSession(self)
        os.execute('test -x /usr/bin/elogger && elogger 10 "WEBUI login failed"')
        return
    end
...
function StatusHandler:get(url)
    ...
    self:write(turbo.escape.json_encode(response))
    self:write(");")
    self:set_header("Content-Type", "text/javascript; charset=UTF-8")
end

-- Clear log message files
local ClearLogHandler = class("ClearLogHandler", SessionRequestHandler)
...
local RdbCmdHandler = class("RdbCmdHandler", SessionRequestHandler)
...
local TopMenuHandler = class("TopMenuHandler", turbo.web.RequestHandler)
function TopMenuHandler:get(url)
    csrf_session.prepareSession(self)
    if not top_menu_entries[url] then
        error(turbo.web.HTTPError(404))
    end
    ...
    if isLogin then
        for _, entry in ipairs(top_menu_entries[url]) do
            if assertValidGroup(self, entry, "get", true) then
                redirectUrl = "/"..entry
                break
            end
        end
...
-- Set CSP header to allow script from the default domain address
function MustacheHandler:prepare()
    if #defLanIpAddr == 0 then
        defLanIpAddr = getDefConfLanRdbs('factory', self.request.host)
    end
    self:set_header("Content-Security-Policy", "default-src 'self'; script-src 'self' "..defLanIpAddr.." 'unsafe-inline'; style-src 'self' 'unsafe-inline'; form-action 'self'; frame-ancestors 'self'")
end
...
function MustacheHandler:get(url)
    ...
    local isLogin = validateLogin(self)
    if not AUTH_TABLE[url] then
        turbo.log.debug("Unrecognized url, redirect to index.html")
        self:redirect("/index.html", true)
        return
    end
    local authenticatedOnly = needsAuthentication(url)
    if isLogin and authenticatedOnly then
        assertValidGroup(self, url, "get")
    end

    local isValidHost = validateHost(self)
    if (not isValidHost) then
        turbo.log.error("Host or Referer is not valid")
        error(turbo.web.HTTPError(403))
    end
    ...
    self:write(turbo.web.Mustache.render(substRdb(fileContent),
        {
            ...
            loggedIn=isLogin,
            lang=language_selection.select_language(self),
            csrfToken=csrfToken,
        })
    )
    csrf_session.updateSession(self)
...
local function getObjectHandler(appHandler, action, objName, queryParams, response)
    ...
    if file_exists(fileName) then
        dofile(fileName)
        if objHandler then
            if needsAuthentication(objName) then
                assertValidLogin(appHandler)
                assertValidGroup(appHandler, objName, action)
            end
...
local function getObject(appHandler, objName, queryParams, response)
    ...
    if objHandler then
        response[objName] = objHandler.get(true, appHandler)
...
local FwUpgradeStatusHandler = class("FwUpgradeStatusHandler", turbo.web.RequestHandler)

-- only for status checking after fw upgrade
-- do not check cookies/csrf here
function FwUpgradeStatusHandler:get(url)
    self:add_header("Content-Type", "text/javascript; charset=UTF-8")
...
local UploadHandler = class("UploadHandler", SessionRequestHandler)

function UploadHandler:getUrl(url)
    return url..".html"
end

function UploadHandler:getCsrfToken(url)
    return self:get_argument("csrfTokenPost", "")
end
...
function UploadHandler:post(url)
    ...
    -- file will be marked as 'already processed' in large file upload
    -- in which case, do not write file content
    -- ref: http://gitlab.corp.netcomm.com.au/Saturn/OpenWRT-Phase1/-/merge_requests/140
    if file and "ERROR" ~= file and "already processed" ~= file then
    ...
    local processed_message, extra_messages, ping_url, ping_delay, result_code = mod.post(name, tonumber(commit), target)
    local err_tbl = {
        ["1"] = "An unexpected error occurred; firmware upgrade failed.",
        ["2"] = "The firmware image you have provided is corrupt.",
        ["3"] = "The firmware image you have provided is not compatible with this product."
    }
    if err_tbl[result_code] then
        processed_message = err_tbl[result_code]
    end
    if not processed_message then
        error(turbo.web.HTTPError(403, string.format("Illegal file name %s", name)))
    end
    self:write(string.format('{"result":"%s", "message":"%s", "ping_url":"%s", "ping_delay":%d, "extras":"%s"}',
                                result_code and result_code or "0", processed_message, ping_url, ping_delay or 0, extra_messages))
end

local RootPasswdHandler = class("RootPasswdHandler", turbo.web.RequestHandler)

function RootPasswdHandler:post(url)
    local passwd = self:get_argument("passwd", '')
    local encryptedPassword = luardb.get("admin.user.root.encrypted")
    local bits = encryptedPassword:explode('$')
    local cmd = string.format("echo -n '%s' | openssl enc -a -A -d | openssl passwd -stdin -%s -salt '%s'",
                              passwd, bits[2], bits[3])
    local handle = io.popen(cmd, 'r')
    if not handle then
...
local TimeSyncHandler = class("TimeSyncHandler", turbo.web.RequestHandler)

function TimeSyncHandler:get(url)
    local newTime = self:get_argument("newTime", '')
    turbo.log.debug("TimeSyncHandler("..newTime..")")
    luardb.set("system.browser.time", newTime)
    self:write({})
end
...
function turbo.web.RequestHandler:write_chunked_file(file)
    local sz = 1024*32
...
local handlers = {
    {"%.%.", DotHandler},
    {"^/language_selection$", language_selection.ReqHandler},
    {"^/(login)$",LoginHandler},
    {"^/(logout)$",LoginHandler},
    {"^/(web_server_status)$",StatusHandler},
    {"^/(network_status)$",NetworkStatusHandler},
    {"^/(logfile)/clear_log$",ClearLogHandler},
    {"^/rdb_command/(.*)$",RdbCmdHandler},
    {"^/cgi%-bin/(.*)$", CgiHandler},
    {"^/fw_status/(.*)$", FwUpgradeStatusHandler},
    {"^/upload/(.*)$", UploadHandler},
    {"^/root_pass$", RootPasswdHandler},
    {"^/top_menu/(.*)$", TopMenuHandler},
    {"^/time_sync$", TimeSyncHandler},
    {"^/$", MustacheHandler},
...
-- can estimate booting time.
-- record the first uptime only.
...
if not luardb.get("system.startup_duration") then
...
for i = 1, #loginAccounts do
    loginAccountsTbl[loginAccounts[i]] = true
end

...
local function upgradeFirmware(uploaded_file, uploaded_commit)
    ...
    -- Run dry test first
    local cmd = string.format("flashtool '%s' %s --test", uploaded_file, option)
    os.execute(cmd)
    local result_code = luardb.get("service.updating_software.result_code") or "0"
    if result_code ~= "0" then
        return "", "", "", 0, result_code
    end
    cmd = string.format("flashtool '%s' %s &", uploaded_file, option)
    os.execute(cmd)
    ...
    return processed_message, extra_messages, ping_url, ping_delay, result_code
end

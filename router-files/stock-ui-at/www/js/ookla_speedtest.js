(function (window, $) {
    ...
    function buildGaugeBadgeHtml(kind) {
        var stage = state.live && state.live.stage ? String(state.live.stage) : "";
        var progress = Number(state.live && state.live.progress || 0);
        var classes = ["ookla-gauge-badge"];
        var label = "Idle";
        ...
        if (kind === "download") {
            ...
            } else if (state.phase === "failed" && (stage === "upload" || progress > 0.2 || Number(state.live && state.live.download_mbps || 0) > 0)) {
                classes.push("is-done");
                label = "Download completed";
            } else if (state.running && (stage === "download" || stage === "upload")) {
            ...
            if (state.phase === "completed") {
                classes.push("is-done");
                label = "Finished cleanly";
            } else if (state.phase === "failed" && stage === "upload" && progress > 0.9) {
                classes.push("is-failed");
                label = "Upload did not finish cleanly";
            } else if (state.running && stage === "upload") {
                classes.push("is-live");
                label = "Upload active";
            }
        }

        return "<div class='" + classes.join(" ") + "' title='" + escapeHtml(label) + "'></div>";
    }

    function buildGaugeHtml(title, value, scale, accentClass, subtitle, gaugeKind) {
        return [
            "<div class='ookla-gauge-card ", accentClass, "'>",
            "<div class='ookla-gauge-head'><div class='ookla-gauge-kicker'>", escapeHtml(title), "</div>", buildGaugeBadgeHtml(gaugeKind), "</div>",
    ...
    function buildIdleGaugeHtml(title, accentClass, subtitle) {
        var gaugeKind = accentClass === "is-upload" ? "upload" : "download";
        return [
    ...
    function buildCaBandsHtml() {
        ...
        return [
            ...
            (bands.length ? bands.map(function (band, index) {
                return "<span class='ookla-ca-pill" + (index === 0 ? " is-primary" : "") + "'>" + escapeHtml(band) + "</span>";

...
try {
    ...
    $releaseOoklaBundleDir = Join-Path $releasePackageRoot "usrdata\at-stock-ui\bundles\ookla"
    if (Test-Path $releaseStockSnapshots) {
        Remove-Item -Recurse -Force $releaseStockSnapshots
    }
    if (Test-Path $releaseOoklaBundleDir) {
        Remove-Item -Recurse -Force $releaseOoklaBundleDir
    }
